...
public class ExtendedPlayer implements IExtendedEntityProperties
{
	...
	public final void saveNBTData(NBTTagCompound compound) {
		...
		properties.setInteger("CurrentChakra", player.getDataWatcher().getWatchableObjectInt(CHAKRA_WATCHER));
	...
	public final void loadNBTData(NBTTagCompound compound) {
		...
		player.getDataWatcher().updateObject(CHAKRA_WATCHER, properties.getInteger("CurrentChakra"));
		maxChakra = properties.getInteger("MaxChakra");
		chakraRegenTimer = properties.getInteger("ChakraRegenTimer");
		System.out.println("[TUT PROPS] Chakra from NBT: " + player.getDataWatcher().getWatchableObjectInt(CHAKRA_WATCHER) + "/" + this.maxChakra);
	}
	...
	public final void setCurrentChakra(int amount) {
		player.getDataWatcher().updateObject(CHAKRA_WATCHER, amount > 0 ? (amount < maxChakra ? amount : maxChakra) : 0);
	}
	...
	public final void setMaxChakra(int amount) {
		...
		//NarutoCraftMod.packetPipeline.sendTo(new SyncPlayerPropsPacket(player), (EntityPlayerMP) player);
	}
