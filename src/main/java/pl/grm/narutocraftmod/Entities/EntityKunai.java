 ...
 */
	...
	/**
	 * returns true if player can attack with this EntityItem in Hand
	 */
	@Override
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
		if (!this.worldObj.isRemote && this.inGround && EntityKunai.kunaiShake <= 0) {
	...
	public void onEntityUpdate() {
		...
		if (block.getMaterial() != Material.air) {
			block.setBlockBoundsBasedOnState(this.worldObj, this.d, this.e,
					this.f);
			AxisAlignedBB axisalignedbb = block
					.getCollisionBoundingBoxFromPool(this.worldObj, this.d,
							this.e, this.f);
		...
		if (EntityKunai.kunaiShake > 0) {
			--EntityKunai.kunaiShake;
		}
		...
		if (this.inGround) {
			...
			vec31 = this.worldObj.getWorldVec3Pool().getVecFromPool(this.posX,
					this.posY, this.posZ);
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (this.shootingEntity == null) {
						damagesource = DamSource.causeKunaiDamage(this, this);
					} else {
						damagesource = DamSource.causeKunaiDamage(this,
								this.shootingEntity);
					}
					...
					this.playSound("random.bowhit", 1.0F,
							1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
					this.inGround = true;
					EntityKunai.kunaiShake = 0;

					if (this.field_145790_g.getMaterial() != Material.air) {
						this.field_145790_g.onEntityCollidedWithBlock(
								this.worldObj, this.d, this.e, this.f, this);
					}
				}
			}
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		EntityKunai.kunaiShake = par1NBTTagCompound.getByte("shake") & 255;
		...
		if (par1NBTTagCompound.hasKey("pickup", 99)) {
			...
			this.canBePickedUp = par1NBTTagCompound.getBoolean("player")
					? 1
					: 0;
		}
	}
	...
	public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		par1NBTTagCompound.setByte("shake", (byte) EntityKunai.kunaiShake);
