...
public class PacketNinjaStatsRequest implements IMessage
{
	...
	public static class PacketNinjaStatsRequestHandler implements IMessageHandler<PacketNinjaStatsRequest, IMessage> 
	{
		...
		{			
			...
			{
				return new PacketNinjaStatsResponse(ExtendedProperties.get(ctx.getServerHandler().playerEntity).psa.getValues());
			}
			else if (message.command.equals("set"))
			{
				...
				//Check to make sure the proper amount of skill points where used
				if (totalPointsUsed <= ((message.data[7] - 1) * PlayerSkillsAtrributes.skillPointsPerLevel))
					ExtendedProperties.get(ctx.getServerHandler().playerEntity).psa.setValues(message.data);
				else //Dirty Cheater or faulty information, reset values. Try again.
					ExtendedProperties.get(ctx.getServerHandler().playerEntity).psa.setValues(resetValues(message.data));
		...
		{
			//These values are to reset all values if someone is 'cheating' aka memory hacking
