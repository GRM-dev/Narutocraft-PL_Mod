...
public class PacketNinjaStatsRequest implements IMessage {
	public static class PacketNinjaStatsRequestHandler
			implements
				IMessageHandler<PacketNinjaStatsRequest, IMessage> {

		@Override
		public IMessage onMessage(PacketNinjaStatsRequest message,
				MessageContext ctx) {
			if (message.command.equals("get")) {
				...
				totalPointsUsed += (message.data[0] + message.data[1]
						+ message.data[2] + message.data[3]) * 3;
