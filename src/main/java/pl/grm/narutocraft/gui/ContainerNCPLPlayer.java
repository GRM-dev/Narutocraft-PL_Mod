...
public class ContainerNCPLPlayer extends Container {
	...
	public ContainerNCPLPlayer(EntityPlayer player, InventoryPlayer inventoryPlayer,
		...
		if ((slot != null) && slot.getHasStack()) {
			...
			if (par2 < INV_START) {
				...
				if (itemstack1.getItem() instanceof Jutsu) {
					if (!this.mergeItemStack(itemstack1, 0, JutsuInv.INV_SIZE, false)) { return null; }
				}
				// if item is armor
				else if (itemstack1.getItem() instanceof ItemArmor) {
					int type = ((ItemArmor) itemstack1.getItem()).armorType;
					if (!this.mergeItemStack(itemstack1, ARMOR_START + type, ARMOR_START
							+ type + 1, false)) { return null; }
				}
				// item in player's inventory
				else if ((par2 >= INV_START) && (par2 < HOTBAR_START)) {
					// place in action bar
					if (!this.mergeItemStack(itemstack1, HOTBAR_START, HOTBAR_START + 1,
							false)) { return null; }
				}
				// item in action bar
