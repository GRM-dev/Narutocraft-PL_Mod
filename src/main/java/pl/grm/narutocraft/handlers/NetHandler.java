...
public class NetHandler {
	...
	public void init() {
		Channel = NetworkRegistry.INSTANCE.newEventDrivenChannel("NCPLDataTunnel");
	}
	...
	public void sendShapeGroupChangePacket(int newCastingMode, int entityid) {
		byte[] packetData = new DataWriter().add(newCastingMode).add(entityid).generate();
	...
	public void sendSpellApplyEffectToAllAround(EntityLivingBase caster, Entity target,
		...
		sendPacketToAllClientsNear(world.provider.dimensionId, x, y, z, 64.0D, (byte) 44,
				writer.generate());
	}
	...
	public void sendStarImpactToClients(double x, double y, double z, World world,
		...
		sendPacketToAllClientsNear(world.provider.dimensionId, x, y, z, 64.0D, (byte) 48,
				writer.generate());
	}
	...
	public void syncWorldName(EntityPlayerMP player, String name) {
		sendPacketToClientPlayer(player, (byte) 47, new DataWriter().add(name).generate());
	}
}
