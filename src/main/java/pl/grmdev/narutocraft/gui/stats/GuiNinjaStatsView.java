...
import pl.grmdev.narutocraft.gui.GuiNinjaButton;
import pl.grmdev.narutocraft.libs.config.References;
import pl.grmdev.narutocraft.player.Attributes;
...
public class GuiNinjaStatsView extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		for (int attrIndex = 0; attrIndex < Attributes.values().length; attrIndex++) {
			...
			drawDefaultString("" + attrb.getAttValues().get(Attributes.values()[attrIndex].getSName()), this.startLeft
					+ this.padLeft + 110, this.startTop + this.padTop + (9 * (2 + attrIndex)), 0xddeeee);
			...
			if (stats.getSkillPoints() >= 2) {
				this.buttonList.add(new GuiNinjaButton(attrIndex, this.startLeft + this.padLeft + 145, this.startTop
						+ this.padTop + (9 * (2 + attrIndex)), "plus"));
			}
		}

		this.drawString(this.fontRendererObj, "Ninja Level: " + stats.getNinjaLevel(), this.startLeft + this.padLeft,
				(this.startTop + this.padTop) - 9, 0xddeeee);
		this.drawString(this.fontRendererObj, "Skill Points: " + stats.getSkillPoints(), this.startLeft + this.padLeft,
				this.startTop + this.padTop, 0xddeeee);
		...
		this.buttonList.add(new GuiButton(Attributes.values().length + 1, this.startLeft + this.padLeft + 50,
				this.startTop + this.padTop + (9 * 20), 50, 20, "Reset"));
