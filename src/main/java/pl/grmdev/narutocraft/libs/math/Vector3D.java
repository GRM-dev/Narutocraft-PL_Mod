...
public class Vector3D {
	...
	public static Vector3D readFromNBT(NBTTagCompound compound) {
		return new Vector3D(compound.getFloat("Vec3_x"), compound.getFloat("Vec3_y"), compound.getFloat("Vec3_z"));
	}
	...
	public boolean isWithinRange(float min, float max) {
		return (this.x >= min) && (this.x <= max) && (this.y >= min) && (this.y <= max) && (this.z >= min)
				&& (this.z <= max);
	}
